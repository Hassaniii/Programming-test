...
public class ConfigController : MonoBehaviour {
	...
	public GameObject startButton;			//the actual start button!
	...
	void Awake (){
        ...
        p1AimBar.transform.localScale = new Vector3(TeamsManager.getTeamSettings(p1TeamCounter).z / barScaleDivider,
                                                     p1AimBar.transform.localScale.y,
                                                     p1AimBar.transform.localScale.z);
        ...
        p2AimBar.transform.localScale = new Vector3(TeamsManager.getTeamSettings(p2TeamCounter).z / barScaleDivider,
                                                     p2AimBar.transform.localScale.y,
                                                     p2AimBar.transform.localScale.z);
	...
	void Update (){	
		...
		if (!p1TeamLock.activeSelf && !p2TeamLock.activeSelf && !p1FormationLock.activeSelf && !p2FormationLock.activeSelf) {
			...
			startButton.GetComponent<Renderer> ().material.color = new Color (1, 1, 1, 1);
		} else {
			startButton.GetComponent<BoxCollider> ().enabled = false;
			startButton.GetComponent<Renderer> ().material.color = new Color (1, 1, 1, 0.3f);
		}
	}
		...
		if (Physics.Raycast(ray, out hitInfo)) {
			...
			switch(objectHit.name) {
                ...
                case "Field-TBL":
                    ...
                    selectedFieldIndex--;
                    fixCounterLengths();
                    fieldIcon.GetComponent<Renderer>().material = availableFields[selectedFieldIndex];
                    break;
	...
	void checkFormationLockState(GameObject formationLock, int formationCounter) {
		...
		print(formationLock.name + " (" + formationCounter + ") state is: " + FormationManager.getFormationInfo(formationCounter).y);
		if (FormationManager.getFormationInfo(formationCounter).y == 1) {
